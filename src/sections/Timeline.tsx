...
const ERAS: Era[] = [
  ...
  },
  {
    ...
    note: 'This is the model you build in part 1. It reads a fixed number of words and no more.',
  },
  {
    year: '2013',
    short: '13',
...
export function Timeline() {
  ...
  return (
    <div className="figure">
      ...
      {/* every entry is rendered into the same grid cell, so the panel keeps the
          height of the tallest one and does not resize while the slider moves */}
      <div className="era-stack">
        {ERAS.map((era, n) => (
          <div key={era.year} className={`era-pane${n === i ? ' on' : ''}`} aria-hidden={n !== i}>
            <div className="era-head">
              <span className="era-year">{era.year}</span>
              <span className="era-name">{era.name}</span>
            </div>
            <p className="era-how">{era.how}</p>
            <div className="era-sample">
              <div className="era-row">
                <span className="era-tag">in</span>
                <span className="era-val">{era.sample.prompt}</span>
              </div>
              <div className="era-row o">
                <span className="era-tag out">out</span>
                <span className="era-val">{era.sample.output}</span>
              </div>
            </div>
